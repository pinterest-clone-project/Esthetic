...
export const useAdminBoardsFilters = () => {
    ...
    const sortBy = (searchParams.get("sortBy") as BoardSortBy) ?? "CreatedAt";
    const sortDirection = (searchParams.get("sortDirection") as SortDirection) ?? "Desc";
    const sortValue = `${sortBy}:${sortDirection}` as `${BoardSortBy}:${SortDirection}`;
    ...
    const handlePageSizeChange = (newSize: number) => updateParams({ pageSize: String(newSize), page: "1" });
