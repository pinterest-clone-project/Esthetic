...
export const useAdminCategoriesFilters = () => {
    ...
    const sortBy = (searchParams.get("sortBy") as CategorySortBy) ?? "CreatedAt";
    const sortDirection = (searchParams.get("sortDirection") as SortDirection) ?? "Desc";
    const sortValue = `${sortBy}:${sortDirection}` as `${CategorySortBy}:${SortDirection}`;
