            ...
            <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                    <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-800 dark:text-white/80 truncate">{category.name}</span>
                        <span className="shrink-0 text-xs text-gray-500 dark:text-white/40 bg-gray-200 dark:bg-white/8 px-2 py-0.5 rounded-full">
                            {t('categories.pinsCount', { count: category.pinsCount })}
                        </span>
                    </div>
                    <span className="text-xs text-gray-400 dark:text-white/30 truncate block">{t('categories.slugLink', { slug: category.slug })}</span>
                </div>
            </div>
