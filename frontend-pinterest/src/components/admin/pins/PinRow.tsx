...
const PinRow = ({ pin, onEdit, onDelete }: PinRowProps) => {
    ...
    <div className="flex items-center justify-between gap-3 bg-white/5 border border-white/8 rounded-2xl px-4 py-3">
        <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                    <span className="text-sm text-white/80 truncate">{pin.title || t('pins.untitled')}</span>
                    {pin.categoryName && (
        ...
        >
            <Pencil size={16} />
        </button><button onClick={() => onDelete(pin)} className="p-2 rounded-xl bg-red-500/10 text-red-300 hover:bg-red-500/20 transition-colors" aria-label={t('pins.delete')}><Trash2 size={16} /></button></div>
    </div>);
};
