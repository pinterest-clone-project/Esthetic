...
export const PinReportGroupCard = ({ group }: { group: IPinReportGroup }) => {
    ...
    return (
        <div className="bg-[#18181b]/90 border border-white/10 rounded-2xl overflow-hidden transition-all duration-200 shadow-lg shadow-black/20">
            {/* Header Card Button */}
            <button
                onClick={handleExpand}
                className="w-full flex items-center gap-4 p-4 text-left hover:bg-white/[0.03] active:bg-white/[0.05] transition-colors"
            >
                {/* Pin Image / Placeholder */}
                <div className="relative w-12 h-12 rounded-xl bg-white/5 border border-white/10 overflow-hidden shrink-0 flex items-center justify-center">
                    {group.pinImage ? (
                        <img
                ...
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    {/* Chevron icon */}
                    <div className={`p-1 rounded-lg text-white/40 transition-transform duration-200 ${expanded ? "rotate-180 bg-white/5 text-white/80" : ""}`}>
            ...
            {expanded && (
                <div className="border-t border-white/10 bg-black/20">
                    {error && (
                        <div className="px-4 py-2.5 text-xs font-medium text-red-400 bg-red-500/10 border-b border-red-500/20 flex items-center gap-2">
                    ...
                    <div className="divide-y divide-white/5 px-2">
                        {localReports.map((report) => (
                            <div key={report.id} className="p-3 my-1 rounded-xl hover:bg-white/[0.02] transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3">
